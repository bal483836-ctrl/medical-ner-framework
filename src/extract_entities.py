...
def build_cmeee_prompt(text: str, few_shot_str: str) -> str:
    """CMeEE prompt (v4.4 强化版)：明确边界规则、强制单字、前缀完整保留、ite/equ/pro 强化"""
...
⑥ 边界精确：不带尾部「了/的/吗/啊」
⑦ ⭐【前缀修饰语完整保留】凡是「原因不明的/疑似/可能/慢性/急性/严重/轻度/中度/重度/反复/持续/原发性/继发性/特发性/遗传性/先天性/获得性/左侧/右侧/双侧/远离X的」+实体，整体作为一个实体抽出，**不要剥离修饰语**：
    - 正例：「原因不明的肾功能减退」整体抽（不要只抽「肾功能减退」）
    - 正例：「慢性小管间质性疾病」整体抽
    - 正例：「持续发热1个月以上」整体抽
    - 正例：「远离大动脉的室间隔缺损」整体抽
⑧ ⭐【ite 指标类必抽】颅内压/胃内压/胃排空/潮气量/肺活量/肺泡通气量/血容量/尿常规/血常规/心率/血压/血糖/体温/白细胞计数/红细胞计数/血气分析 等指标项作为 ite 抽出
⑨ ⭐【pro 操作类必抽】叩诊/触诊/视诊/听诊/环切/口服/注射/灌肠/插管/活检/穿刺/CT/MRI/X线/B超/心电图 等检查与操作作为 pro 抽出（即使只有 2 字）
⑩ ⭐【equ 设备类必抽】呼吸机/内镜/光镜/腹腔镜/导尿管/胃管/弹簧圈/支架/袖带/监测仪 等器材作为 equ 抽出
⑪ ⭐【完整 dis 名优先】疾病名遇到完整描述形式应整体抽：「巨细胞病毒感染」「口腔念珠菌病」「带状疱疹」「呼吸道感染」整体抽
