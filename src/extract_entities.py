...
def build_cmeee_prompt(text: str, few_shot_str: str) -> str:
    """CMeEE prompt (v4.5 精简版)：核心 7 条规则，避免规则疲劳"""
    types = "、".join([f"{v}({k})" for k, v in CMEEE_TYPE_MAP.items()])
    return f"""医学 NER 任务。从文本逐字符抽出**全部**医学实体，原文复制，逗号分隔。
**宁可多抽不可漏抽**（嵌套都抽：长形式和短形式同时输出）。

实体类型：{types}

# 7 条核心规则
① 嵌套都抽：「葡萄球菌肺炎」和「葡萄球菌」都抽；「胃肠功能疾病」和「胃」「肠」都抽
② 单字身体部位独立抽：脑/心/肝/肺/肾/胃/肠/血/尿/胸/腹/头/眼/耳/口/手/脚/骨/皮/喉/腰/鼻/舌/齿
③ 前缀修饰语保留：「原因不明的肾功能减退」整体抽 **同时** 也抽「肾功能减退」；「慢性肠胃炎」整体抽 **同时** 也抽「肠胃炎」
④ 后缀去掉到 CMeEE 边界：「头痛」抽「头」，「瞳孔变化」抽「瞳孔」（变化/增高/降低/紊乱/异常 类后缀剥离）
⑤ 并列短语整体保留：「水、盐电解质紊乱」「左、右心室」整体抽
⑥ 各类型完整抽：
    - dis 疾病（炎/症/病/癌/瘤 等）：肺炎/糖尿病/带状疱疹/口腔念珠菌病
    - sym 症状（描述身体异常）：头痛/咳嗽/水肿/呼吸加速/精神差/共济失调
    - pro 操作（检查/治疗）：叩诊/CT/MRI/口服/注射/活检/化疗
    - ite 指标：颅内压/胃排空/潮气量/肺活量/血压/血糖/白细胞计数
    - equ 设备：呼吸机/内镜/导尿管/弹簧圈/袖带
    - dru 药物：阿司匹林/青霉素/维生素A
    - bod 部位/mic 微生物/dep 科室
⑦ 禁抽（这些不是 CMeEE 的标注对象，抽了算错）：
    - 通用学科名：生理学/解剖学/病理学/基础学/精神社会学/医学/心理学
    - 通用概念：症状/疾病/表现/状态/结果/原因/方法/影响/情况/部分/作用
    - 通用动作：治疗/诊断/检查/观察/分析/研究/制定/标准/委员会
    - 模糊词：可能/相关/异常/正常/常见/严重/明显/进一步/具体
    - HTML 标签（<sub>/<sup>等）
    - 性别年龄（男/女/患者/患儿/新生儿/婴儿/儿童）
    - 时间数字单位（年/月/天/mg/kg/ml/℃/%）
    - 含「了/的/吗/啊」尾助词
...
实体（逗号分隔，**嵌套都抽，宁多勿漏，但严禁抽通用概念/学科名**）："""
...
def build_imcs_turn_prompt(
    ...
    """
    return f"""儿科对话NER任务。只从【当前发言】里提取**患者的症状/体征**，原文原样输出，逗号分隔。

✅ 必抽（症状/体征）：
- 体征异常：发烧/发烫/咳嗽/喘息/拉肚子/拉稀/吐奶/呕吐/流鼻涕/鼻塞/腹胀/腹痛/喉咙痛
- 描述性体征：绿便/水样便/蛋花汤样便/血便/稀便/绿色的大便/黄痰/清鼻涕
- 微小表现：放屁/打嗝/夜咳/嗓子哑/精神差/没精神/哭闹/抽搐/尿少/出汗/食欲差
- 数字体温：38.5度/37.2℃

🚫 严禁抽（这些不是 IMCS 的标注对象）：
- 药物：蒙脱石散/益生菌/退烧药/退热贴/口服液/妈咪爱/罗红霉素
- 检查项目：血常规/便常规/B超/拍片/CT/水电解质
- 病因诱因：受凉/吹风/吃多了/食积/挑食/腹部受冷
- 患者称谓：宝宝/孩子/患儿/男孩/女孩
- 医务动作：吃药/打针/退烧/喂水/服用
- 舌象（IMCS 不属症状）：舌苔黄/舌苔厚/舌尖红
- 否定/询问句中的症状：「不发烧」「没拉稀」「有腹泻吗？」「会不会发烧」
- 模糊描述：精神状态/大便次数多/特别粘/还可以
- 通用概念：感冒（除非作为患者描述的症状）、疾病、症状
