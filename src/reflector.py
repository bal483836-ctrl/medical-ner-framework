...
def _cot_prompt(text: str, entities: List[str], domain_hint: str) -> str:
    ...
    return f"""你是医学审稿专家。{domain_hint}你的任务是【补充】候选实体列表，使其更完整。
...
# ⚠️ 重要原则（必须严格遵守）
1. **默认保留所有候选实体**——除非该候选明显属于以下硬性禁用情况之一，否则全部保留：
   (a) 候选包含原文中**不存在**的字符（拼写错乱）
   (b) 候选是明显的否定/询问引用（如「不发烧」「未见」「是否有」整段被抽出）
2. **主任务是补充遗漏的实体**：扫描原文，找出候选中**没有**但应该抽出的医学实体加进去
3. **怀疑但不确定的实体一律保留**（宁错不漏）——你不知道的医学术语很可能就是 gold
4. 不要新增原文中没有的实体（不要发明）
5. 不要对候选做归一化/改写（"急性肠胃炎"不要改成"急性胃肠炎"）
...
<answer>
完整实体列表 = 原候选(去掉明显错误的) + 新发现的实体，用顿号"、"分隔
</answer>"""
