...
def _cot_prompt(text: str, entities: List[str], domain_hint: str) -> str:
    ...
    return f"""你是严谨的医学 NER 审稿专家。{domain_hint}
...
# ⚠️ 任务（不是宽松审核，是严谨补充）
你要做两件事：
1. **保留**所有候选实体（默认不删）——除非该候选**明显不符合本数据集的标注范围**（见下方负例）
2. **补充**原文中遗漏的、且**确定是医学实体**、且**符合本数据集标注范围**的实体

# 🚫 严禁补充以下类型（这些不属于本数据集标注范围）
- 通用学科名（生理学/解剖学/病理学/基础学/精神社会学）
- 通用概念（症状/疾病/表现/状态/结果/原因/方法/影响）
- 通用医学动作（治疗/诊断/检查/观察/分析/研究）
- 非医学背景词（数值/时间/地点/人名/产品名）
- 模糊描述（情况/部分/可能/相关/异常/正常）

# 🚫 严禁删除
- 你"不认识"的术语 → 保留（它很可能是 gold）
- 短/单字实体 → 保留（特别是单字身体部位）
- 已经在候选里的，**默认全部保留**

# 输出格式
<thinking>
1. 我准备删除哪些候选？（必须给出充足理由——它明确属于上述负例）
2. 我准备新增哪些实体？（必须满足：① 出现在原文 ② 是具体医学术语 ③ 不属于上述负例）
</thinking>
<answer>
最终实体列表，用顿号"、"分隔
</answer>"""
...
def reflect_cmeee(items, output_path=None):
    ...
    return reflect_batch(
        ...
        domain_hint=(
            "数据集为 CMeEE_V2，**只标注以下 9 类具体的医学命名实体**："
            "dis(疾病/综合征/感染)、sym(具体症状/体征)、pro(操作/手术/检查动作)、"
            "equ(医疗器械/设备)、dru(药物名称)、ite(医学指标项)、bod(具体身体部位/解剖结构)、"
            "mic(具体微生物/细菌/病毒)、dep(临床科室)。"
            "**通用学科名（生理学/解剖学）、通用概念（症状/表现/原因/方法）、"
            "通用动作（治疗/分析/研究）一律不属于本数据集**，禁止补充这些。"
        ),
...
def reflect_imcs(items, output_path=None):
    ...
    res = reflect_batch(
        ...
        domain_hint=(
            "数据集为 IMCS_V2 儿科医患对话，**只标注【症状/体征】**（如：发烧/咳嗽/拉肚子/"
            "流鼻涕/呕吐/精神差/绿便/水样便/腹胀）。"
            "**严禁补充以下任何类型**：药物名（蒙脱石散/益生菌/退烧药）、"
            "检查项目（血常规/B超）、疾病诊断（肺炎/感冒，除非原文明确作为患者症状描述）、"
            "病因诱因（受凉/吹风/吃多了）、患者称谓（宝宝/孩子）、"
            "医务动作（吃药/打针/退烧）、舌象（舌苔黄）、否定句中的症状（不发烧/没拉稀）、"
            "询问句中的症状（有腹泻吗）。"
            "抽原文形态，**不要归一化**（『拉肚子』保留『拉肚子』，不要写『腹泻』）。"
        ),
        output_path=output_path,
    )
    for it in res:
